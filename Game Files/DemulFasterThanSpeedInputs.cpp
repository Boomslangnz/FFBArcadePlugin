...
static int BoostButton = GetPrivateProfileInt(TEXT("Settings"), TEXT("BoostButton"), 0, settingsFilename);
...
static int BoostButtonDevice2 = GetPrivateProfileInt(TEXT("Settings"), TEXT("BoostButtonDevice2"), 0, settingsFilename);
...
void FasterThanSpeedInputsEnabled(Helpers* helpers)
{
	if (!Inputsinit)
	{
	...
	std::string boost2("BoostButtonDevice2");
	...
	while (SDL_WaitEvent(&e) != 0)
	{
		...
		if (e.jhat.type == SDL_JOYHATMOTION)
		{
			if (e.jhat.value == SDL_HAT_CENTERED)
			{
				if (e.jhat.which == joystick_index1)
				{
					if (testbuttonA)
					{
						helpers->WriteByte(ServiceTestAddress, servicetestread += 0x40, false);
						testbuttonA = false;
					}
					if (servicebuttonA)
					{
					...
					if (boostbuttonA)
					{
						helpers->WriteByte(ShiftUpDownAddress, shiftupdownread += 0x04, false);
						boostbuttonA = false;
					}
					if (startbuttonA)
					{
						helpers->WriteByte(ShiftUpDownAddress, shiftupdownread += 0x08, false);
						startbuttonA = false;
					}
					if (shiftupA)
					{
					...
					if (shiftdownA)
					{
						helpers->WriteByte(ShiftUpDownAddress, shiftupdownread += 0x20, false);
						shiftdownA = false;
					}
					if (coinA)
					{
			...
			if (e.jhat.value == SDL_HAT_LEFT)
			{
				if (e.jhat.which == joystick_index1)
				{
					...
					{
						helpers->WriteByte(ShiftUpDownAddress, shiftupdownread -= 0x80, false);
						startbuttonA = true;
					}
		...
		if (e.type == SDL_JOYBUTTONDOWN)
		{
			if (e.jbutton.which == joystick_index1)
			{
				...
				if (e.jbutton.button == BoostButton)
				{
					helpers->WriteByte(ShiftUpDownAddress, shiftupdownread -= 0x04, false);
				}
			...
			else if (e.jbutton.which == joystick_index2)
			{
				...
				if (e.jbutton.button == BoostButtonDevice2)
				{
					helpers->WriteByte(ShiftUpDownAddress, shiftupdownread -= 0x04, false);
				}
